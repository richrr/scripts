...
def comp_same_mod_two_method_networks_with_edge_attributes(infiles, img_qual, img_frmt, delim, log_file_list, log_file, twomethods):
    infile1 = infile2 = ''
    if len(infiles) == 2:
        infile1 = infiles[0]
        infile2 = infiles[1]
    ...
    writeLIST_to_file(["%s,%s" % tup for tup in common_edges], fign+"-common_edges.txt")
    writeLIST_to_file(["%s,%s" % tup for tup in common_edges_w_diff_correl], fign+"-common_edges_w_diff_correl.txt")
    ...
    # write to file: (i) common edges (ii) common edges with diff. correlations
    writeLIST_to_file(["%s,%s" % tup for tup in sm_common_edges], fign+"-SM_common_edges.txt")
    writeLIST_to_file(["%s,%s" % tup for tup in sm_common_edges_w_diff_correl], fign+"-SM_common_edges_w_diff_correl.txt")
    ...
    if twomethods:
        ...
        writeLIST_to_file(two_method_edges, fign+"-SM_common_edges-two_method_edges.txt")

    if not twomethods: # directly go to plot networks
        run_cmds = '\nRun two commands: \n'
        for (e, n) in [(infile1, node_attrib_file1) , (infile2, node_attrib_file2)]:
            run_cmds += 'python ~/scripts/python/plot-network-graph.py -i %s -n %s -a ?_node_abundance.txt -c ?-SM_common_edges.txt -b ?-SM_common_edges_w_diff_correl.txt\n' %(e, n)
        print run_cmds
...
def main(args):
    ...
    parser.add_argument('-c', '--comparetwomethodsnets', action='store_true', default=False) # compare same module common edges (nets) of differents group from two methods. P.S. you have to run the script twice with -t and then finally once with -c and -t
    ...
    elif args.comparetwomethodsnets: #if this is true, ignore the node attribute files
        comp_same_mod_two_method_networks_with_edge_attributes(infiles, img_qual, img_frmt, delim, log_file_list, log_file, args.twomethods)
    elif args.nodefiles != None:
        node_attrib_files = args.nodefiles
        comp_networks_with_edge_attributes(infiles, node_attrib_files, img_qual, img_frmt, delim, log_file_list, log_file, args.twomethods)
...
def comp_networks_with_edge_attributes(infiles, node_attrib_files, img_qual, img_frmt, delim, log_file_list, log_file, twomethods):
    infile1 = infile2 = ''
    if len(infiles) == 2:
        infile1 = infiles[0]
        infile2 = infiles[1]
