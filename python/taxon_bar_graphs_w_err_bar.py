...
def main(args):
    ...
    logfile = infile + '_' + '_'.join(categories_to_print) + '_' + 'tax-plot-err-bar-log-file.txt'
    logfilelist = list()
...
def draw_tax_plots_w_err(c, mapping_file, lines, infile, outpfile, delim, f_type, contrasts, p_val_adj, variance, stats_test):
    ...
    #category_list_sorted = ["A-V0", "D-V0", "A-V2", "D-V2"]
    #category_list_sorted = ["A-V2", "D-V2", "A-E3", "D-E3"]
    #category_list_sorted = ["A-V0", "D-V0", "A-V2", "D-V2", "A-E3", "D-E3"]
    #category_list_sorted = ["Dune7a", "0", "105", "155", "181", "210", "392", "450", "517", "845", "1475", "2385", "3210", "4010", "6500"] # haast-michigan data
    #category_list_sorted = ["0", "105", "155", "210", "450", "845", "1475", "2385", "3210", "4010"] # michigan only from haast-michigan data
    #category_list_sorted = ["Dune7a", "181", "392", "517", "6500"] # haast only from haast-michigan data
    ...
    for c in contrasts:
        ...
        if stats_test == 'manwu':
            ...
            if p_val_adj == 'bf':
                ...
                if pval_ <= 0.05:#unadjusted
                    str_to_output = "%s\t%s\tmanwu-stat\t%.3f\t p-val\t%.3f\tbf adj p-val(contrasts)\t%.3f\tbf adj p-val(taxa)\t%.3f\tbf adj p-val(contrasts & taxa)\t%.3f" % (c, taxon_name, mstats, pval_, pval_adjusted_c, pval_adjusted_t, pval_adjusted)
                    print str_to_output
                    logfilelist.append(str_to_output)
            continue
        ...
        if variance == 'unequal':
            ...
            if p_val_adj == 'bf':
                ...
                pval_adjusted = pval_ * len(contrasts) * numb_taxa
                if pval_ <= 0.05:#unadjusted
                    str_to_output = "%s\t%s\tt-stat\t%.3f\t p-val\t%.3f\tbf adj p-val(contrasts)\t%.3f\tbf adj p-val(taxa)\t%.3f\tbf adj p-val(contrasts & taxa)\t%.3f" % (c, taxon_name, tstats, pval_, pval_adjusted_c, pval_adjusted_t, pval_adjusted)
                    print str_to_output
                    logfilelist.append(str_to_output)
            continue
