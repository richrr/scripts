...
def main(args):
    ...
    parser.add_argument('-c', '--categories' , nargs='*') \
        # creates a list of categories to be summarized (enter as space delimited on cmd line), currently only one category with TWO GROUPS ONLY (e.g. A vs D)
...
def shuffle_dict(iternumb): ## takes iternumb which is number of random iteration
    ...
    # divide the shuffled values in two groups as per original sizes. IF you have more than 3 groups, depending on which of the group is returned
    #first, your original and shuffled data may not have the same number of samples. So only keep 2 groups in mapping file
...
def compile_results(iternumb): # get unique elements from last column (otus) from the core_otu_100 files
    core_otus_file = './%s/core_otus_%s.txt' %(iternumb, FRAC_SAMPLES)  # No need for global declaration to read value of globvar
    taxon_list = list()
...
def calc_significance(c, mapping_info_list, lines, f_type, p_val_adj, group_file):
    ...
    # randomize n times and calc. randomized core microb
    print 'Running randomized data:'
    pool.map(shuffle_dict, range(NTIMES))  # do shuffle_dict n times with pool
    
    global FRAC_SAMPLES  ## Needed to modify global copy of globvar
    ...
    for frac_s in [75, 80, 85, 90, 95, 100]:
        #print str(frac_s)
        FRAC_SAMPLES = frac_s
        true_results_otu_list = compile_results(original_data) # compile original
        ...
        writeDICT_to_file(randomized_otus, OUTPFILE+'-randomized-mapping-file-results-'+str(frac_s)+'-same-randomization-set.txt')
        ...
        for o in true_results_otu_list:
            ...
            else: #This was not observed even a single time in the randomized data, pretty good!
                freq = 1
                if freq/float(NTIMES) < 0.05:
                    otu = '%s\t%s' % (o, freq)
                    print otu
        ...
        writeLIST_to_file(signif_core_microb_otu_list, OUTPFILE+'-signif_core_microb_otu_list-results-'+str(frac_s)+'-same-randomization-set.txt')
