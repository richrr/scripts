...
//			System.err.println("matrixbits2:\t"+matrixbits2);
		...
		if(FILL_MIDDLE_INTERMEDIATE || FILL_MIDDLE_FINAL){
			...
			System.err.println("MiddleTable: \tgap = "+middleTable.gap+"   \tmem = "+middleTable.mem()+"   \tused = "+String.format(Locale.ROOT, "%.3f%%",middleTable.usedFraction()*100));
		}
...
//			}
		...
		System.err.println("Avg Insert:          \t\t"+String.format(Locale.ROOT, "%.1f", (insertSumCorrectTotal+insertSumIncorrectTotal)*1d/(correctCountTotal+incorrectCountTotal)));
		if(FASTQ.PARSE_CUSTOM){
...
//			assert(outbad.contains("#") || sam || fq) : outbad;
		...
		for(int i=0; i<kca.length; i++){
			kca[i].shutdown();
			System.err.println("Table "+i+":\tgap = "+kca[i].gap+"   \tmem = "+kca[i].mem()+"   \tused = "+String.format(Locale.ROOT, "%.3f%%",kca[i].usedFraction()*100));
