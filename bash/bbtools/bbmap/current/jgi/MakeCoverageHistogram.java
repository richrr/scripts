...
//					assert(r.list.size()>0);
		...
		System.out.println("\nTotal Coverage:");
		for(int i=0; i<=max; i++){
			System.out.println(i+"\t"+hist[i]+String.format(Locale.ROOT, "\t%.3f%%", histF[i])+"\t"+histC[i]+String.format(Locale.ROOT, "\t%.3f%%", histCF[i]));
		}

		
		System.out.println("\nPerfect Coverage:");
		for(int i=0; i<=max; i++){
			System.out.println(i+"\t"+phist[i]+String.format(Locale.ROOT, "\t%.3f%%", phistF[i])+"\t"+phistC[i]+String.format(Locale.ROOT, "\t%.3f%%", phistCF[i]));
		}
