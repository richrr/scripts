...
//			assert(false) : "'"+sl.rnameS()+"'";
	...
	public void printOutput(){
		...
		Data.sysout.println(String.format(Locale.ROOT, "\nAverage coverage:                    \t%.2f", depthCovered));
		if(USE_COVERAGE_ARRAYS && calcCovStdev){
			double[] stdev=standardDeviation(list, 0, minscaf);
			Data.sysout.println(String.format(Locale.ROOT, "Standard deviation:                    \t%.2f", stdev[1]));
		}
		Data.sysout.println(String.format(Locale.ROOT, "Percent scaffolds with any coverage: \t%.2f", pctScaffoldsWithCoverage));
		if(USE_COVERAGE_ARRAYS || USE_BITSETS){
			Data.sysout.println(String.format(Locale.ROOT, "Percent of reference bases covered:  \t%.2f", pctCovered));
		}
	}
