...
//		String temp2=temp.replace("%", "FINAL");
		...
		if(externalSort){
			...
			SortByName.mergeAndDump(names, dest, dest2, delete, useSharedHeader);
		}
...
//		outstream.println("in1="+in1+", overhead="+(0.5*(ReadKey.overhead+Clump.overhead)));
		...
		if(in2!=null){
			double[] estimates2=Tools.estimateFileMemory(in2, 200, 0.5*(ReadKey.overhead+Clump.overhead), true);
