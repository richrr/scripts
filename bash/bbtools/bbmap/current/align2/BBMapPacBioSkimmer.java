...
//			BBIndexPacBioSkimmer.QUIT_AFTER_TWO_PERFECTS=false;
	...
	public void setup(){
		...
		if(minid!=-1){
			...
			sysout.println("Set MINIMUM_ALIGNMENT_SCORE_RATIO to "+String.format(Locale.ROOT, "%.3f",MINIMUM_ALIGNMENT_SCORE_RATIO));
		}
...
//		if(paired){BBIndexPacBioSkimmer.QUIT_AFTER_TWO_PERFECTS=false;}
	...
	void printSettings(int k){
		...
		if(verbose_stats>=2){
			...
			sysout.println("Fraction To Remove:    \t"+String.format(Locale.ROOT, "%.4f", (BBIndexPacBioSkimmer.REMOVE_FREQUENT_GENOME_FRACTION ? BBIndexPacBioSkimmer.FRACTION_GENOME_TO_EXCLUDE : 0)));
