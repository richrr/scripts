...
# CGRB command:
# 
# SGE_Batch -c 'R < scripts/blocks/part2_blc_difcor_table.R --no-save FALSE NULL "cgrb" 3 TRUE "Project_03mp" "filter.txt" TRUE "pearson" "pairwise.complete.obs" TRUE "dados1.txt" "dados2.txt" "dados3.txt" "Identifiers.csv"' -r dapfinder_part2 -f 256G -m 256G -q samwise
#
# SGE_Batch -c 'R < scripts/blocks/part2_blc_difcor_table.R --no-save FALSE NULL "cgrb" 2 TRUE "Project_mp03_2datasets" "filter.txt" TRUE "pearson" "pairwise.complete.obs" TRUE " "Gene_Expression_balbc_num.txt" "Gene_Expression_B10A_lit_num.txt" "Identifiers.csv" ' -r part2_imsys_2datasets -f 100G -m 100G -q transkingdom

# tests
# SGE_Batch -c 'R < scripts/blocks/part2_blc_difcor_table.R --no-save FALSE NULL "cgrb" 1 TRUE "Project_test" "filter.txt" TRUE "pearson" "pairwise.complete.obs" TRUE "amostra1.txt" "Identifiers.csv"' -r cor_difcor_imsys_test -f 4G -m 4G -q samwise
# at lab: R < Dropbox/Tese/scripts/DAPfinder/blocks/part2_blc_difcor_table.R --no-save FALSE NULL "lab" 1 TRUE "Project_teste" "filter_teste.txt" TRUE "pearson" "pairwise.complete.obs" TRUE "amostra1.txt" "Identifiers.csv"
######################################################################################################
...
for(j in 1:total_studies)
  data[[study.names[j]]] = t(read.table(study.file[j], row.names = 1, header = TRUE, sep = "\t", stringsAsFactors=F))
