...
# command in cgrb
# 
# SGE_Batch -c 'R < analysis_AE/analysis_AE.R $SGE_TASK_ID 50 1 "total" "cgrb" 10 "yes" "no" "one_table_each" "status.txt" "yes" "in" "done.txt" --no-save' -r degs_analysis_done -t 1-50 -f 8G -m 8G -q samwise
...
rerun=args[12]       #two possible options: "yes" or "no"
select.type=args[13]	# two possible options: "in" or "out" of the rerun_data file
run_data=args[14]     # name of the file with the studies to run
  ...
  path_degs="C:/Users/Lina/Dropbox/Tese/Lina-Andrey/raw-data-BcKO/DEGs" else
    if(local=="lab")
      path_degs="/home/thomasli/Dropbox/Tese/Lina-Andrey/raw-data-BcKO/DEGs" else
        if(local=="cgrb")
          path_degs="/capecchi/pharmacy/morgunlab/lina/imsys" 

setwd(path_degs)

#nao sei pq tem na na base dos degs

degs = read.csv("degs_gbacc.csv",header=TRUE)[1:275,1:3]
degs = data.frame(lapply(degs, as.character), stringsAsFactors=FALSE)
