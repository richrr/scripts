...
# command in cgrb
# 
# SGE_Batch -c 'R < analysis_AE/daps_analysis_AE.R $SGE_TASK_ID 15 1 "total" "cgrb" 10 "one_table_each" "status_nosmp.txt" "yes" "in"  "AE_data_refazer_nosmp.txt" --no-save' -r daps_analysis_nosmp -t 1-15 -f 8G -m 8G -q samwise

# SGE_Batch -c 'R < analysis_AE/daps_analysis_AE.R $SGE_TASK_ID 50 1 "total" "cgrb" 10 "one_table_each" "status.txt" "yes" "out" "AE_data_refazer.txt" --no-save' -r daps_analysis -t 1-50 -f 8G -m 8G -q samwise
...
output.option=args[8]	#two possible options: "one_table_each" and "single table"
status_data= args[9]   # name of the file to save the status for each study
rerun=args[10]       #two possible options: "yes" or "no"
select.type=args[11]	# two possible options: "in" or "out" of the rerun_data file
rerun_data=args[12]	# name of the file with the studies to filter/rerun
...
{
  path="/home/thomasli/Dropbox/Tese/Dados/ArrayExpress"
...
} else  if(local=="cgrb")
{
  ...
  ######################################################################################
  loading.studies.chip3=function(p,pi,rerun,select.type,rerun_data){
...
if(local=="lab")
  if(length(system("ls -F /home/thomasli/Dropbox/Tese/Dados/ArrayExpress | grep 'status.txt'", intern =TRUE))==0)  
...
##################################################################################################################
# reading DAPs
# quando fizer o filtro da direcao das correlacoes no comeco do estudo
# daps=read.csv("daps_gbacc.csv",header=TRUE)
# daps <- data.frame(lapply(daps, as.character), stringsAsFactors=FALSE)

# enquanto nao faco o filtro da direcao das correlacoes no comeco do estudo

if(local=="home")
  path_daps="C:/Users/Lina/Dropbox/Tese/Lina-Andrey/raw-data-BcKO/DAPfinder" else
    if(local=="lab")
      path_daps="/home/thomasli/Dropbox/Tese/Lina-Andrey/raw-data-BcKO/DAPfinder" #else
        if(local=="cgrb")
           path_daps="/capecchi/pharmacy/morgunlab/lina/DAPs/imsys" 

setwd(path_daps)

daps = read.csv(file="DAPs in 3 datasets_6000.csv", header=TRUE)
daps <- data.frame(lapply(daps, as.character), stringsAsFactors=FALSE)

daps=daps[(((daps$cor0_gr1>0)&(daps$cor0_gr2>0)&(daps$cor0_gr3>0))|
             ((daps$cor0_gr1<0)&(daps$cor0_gr2<0)&(daps$cor0_gr3<0)))&
            (((daps$cor1_gr1>0)&(daps$cor1_gr2>0)&(daps$cor1_gr3>0))|
               ((daps$cor1_gr1<0)&(daps$cor1_gr2<0)&(daps$cor1_gr3<0))),][,1:6]

#pair components
compdap=stack(daps,select=5:6)
...
if(local=="cgrb")
{
  studies=loading.studies.chip3(p,pi,rerun,select.type,rerun_data)
...
if(is.character(num_studies)) num_studies=length(studies)
...
for(i in start_studies:num_studies)
{
  ...
  if(length(RData)==1)
  {
    ...
    {
      print("WARNING: ALREADY HAD RData") 
  ...
  if(length(RData)==0)
  {
    ...
    for(j in 1:length(AEset))
    {
      ...
      RDataj=grep(paste(studies[i],"_",j,".RData",sep=""),list.files(path),value=TRUE)
      if(length(RDataj)==1)
  ...
  {  
    ...
    if(j>1)
    {
      RData=grep(paste(studies[i],"_",j,".RData",sep=""),list.files(path),value=TRUE)
      ...
      if(!is.matrix(labels)) control=c(grep(c("control"),labels),grep(c("Control"),labels)) else
	    {
	      ...
	      control[control_index]=c(grep(c("control"),labels[,control_index]),grep(c("Control"),labels[,control_index]))

	      control=which(!is.na(control))
...
#       library(chip,character.only=T)
    ...
    rm(states_index,facs2,facs,fac2,fac,min_num_states,max_smp_index,labels,AEsetnorm,nspec,control,dl,cor,dados,dados_daps)
    if(exists("freq_states")) rm(freq_states)
    
  }
}
# 
# # if(output.option=="one_table_each")
# # {
# #   if(local=="lab")
# #     DAPs_Dif=system("ls -F /home/thomasli/Dropbox/Tese/Dados/ArrayExpress | grep 'DAPs_Dif_single'", intern =TRUE) else
# #       if(local=="cgrb")
# #         DAPs_Dif=system("ls -F /capecchi/pharmacy/morgunlab/lina/ArrayExpress | grep 'DAPs_Dif_single'", intern =TRUE) else
# #           if(local=="home")
# #             DAPs_Dif=system("ls -F C:/Users/Lina/Dropbox/Tese/Dados/ArrayExpress | grep 'DAPs_Dif_single'", intern =TRUE) else
# #   
# #   fdif_dap=read.table(fdif_daps,DAPs_Dif[1],header=T)
# #   
# #   if(length(DAPs_Dif)>1)
# #     for(i in 2:length(Daps_Dif))
# #     {
# #       temp=read.table(fdif_daps,DAPs_Dif[i],header=T)
# #       fdif_dap=merge(fdif_dap,temp,by=c(1,2,3,4,5),all=TRUE)
# #     }
# # 
# # }
# # 
# # write.table(fdif_dap,"DAPs_Dif_table.txt",row.names=F, col.names=T,sep="\t")
# # 
# 
#        
