...
# parameters
#==================================================================================================================
p <- arg_parser('perform bootstrap analyses.R Calculates comparison and correlation analysis using bootstrap')
...
p <- add_argument(p, "--numbBootstrap", help="number of times to bootstrap", default=100) 

p <- add_argument(p, "--excludeGroup", help="the group to be excluded from bootstrap") 
...
numbbootstr = argv$numbBootstrap
...
for (name in names(dict)) {
    ...
    for (numb in 1:numbbootstr){
	    if(length(values) <= sampsize){  
	        print(sample(values)) # a random permutation
	    } else {
	        rnd_samp = sample(values, sampsize) # sample without replacement
	        print(data.frame(rnd_samp,name))
	    }
    }
}
}
...
for (numb in 1:numbbootstr) {
    ...
    outputFile_rnd_numb = paste(c("sampSize-", sampsize, "-numbBootstrap-", numbbootstr, "/", outputFile, "/rnd-", numb, "/results_"), collapse='')
...
print(getwd())
out_str = 'Run the following command in the above directory on Needleman: SGE_Array -c cmds-to-execute.txt -q transkingdom -m 50G -f 750G -b 10'
print(out_str)
